...
def train_model(net, optimizer, criterion, train_loader, val_loader,
                ...
                test_loader=None, test_criterion=None, model_save_path=None, model_idx=None,
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        # Per-group accuracy
        group_accs = np.zeros(correct_by_groups_v.shape)
        for y in range(correct_by_groups_v.shape[0]):
            for s in range(correct_by_groups_v.shape[1]):
                if total_by_groups_v[y][s] > 0:
                    group_accs[y][s] = correct_by_groups_v[y][s] / total_by_groups_v[y][s]
        
        # Worst group accuracy
        mask = total_by_groups_v > 0
        worst_group_acc = group_accs[mask].min()
        avg_group_acc = group_accs[mask].mean()

        print(f"Val Loss:           {val_loss:.4f}")
        print(f"Val Acc:            {val_acc:.4f}")
        print(f"Worst Group Acc:    {worst_group_acc:.4f}")
        print(f"Avg Group Acc:      {avg_group_acc:.4f}")
        print(f"Per-group Accs:\n{group_accs}")


        # Save the best model based on global validation set
        if val_loss < best_val_loss_on_global_val:
            print(f"  Global Val accuracy improved for model {model_idx} {val_acc:.3f}%). Saving model to {model_save_path}")
            best_val_loss_on_global_val = val_loss
        ...
        if log_test_results:
            ...
            robust_test_acc,_ = summarize_acc(correct_by_groups_t,
                                            total_by_groups_t)
            ...
            figpath = os.path.join(args.results_path, f'biasmodel_{model_idx}.png')
...
def train(net, dataloader, optimizer, criterion, args, scheduler=None):
    ...
    if hasattr(dataset, 'dataset'):  # it's a Subset
        targets_all = dataset.dataset.targets_all
        indices = dataset.indices
        targets_s = targets_all['spurious'][indices]
        targets_t = targets_all['target'][indices]
    else:
        targets_s = dataset.targets_all['spurious']
        targets_t = dataset.targets_all['target']
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        if hasattr(dataloader.dataset, 'dataset'):  # Subset
            labels_spurious = [dataloader.dataset.dataset.targets_all['spurious'][ix] for ix in data_ix]
        else:
            labels_spurious = [targets_s[ix] for ix in data_ix]
        # Add this here to generalize NLP, CV models
        out = net(inputs)
        outputs = out[1] if isinstance(out, tuple) else out
        ...
        for i, data in enumerate(tqdm(dataloader)):
            ...
            if testing:
                for ix, s in enumerate(labels_spurious):
                    ...
                    total_by_groups[int(y)][int(s)] += 1
                    if return_losses:
                        loss_val = loss[ix].item() if loss.dim() > 0 else loss.item()
                        losses_by_groups[int(y)][int(s)] += loss_val
            inputs = inputs.to(torch.device('cpu'))
            labels = labels.to(torch.device('cpu'))
            outputs = outputs.to(torch.device('cpu'))
            loss = loss.to(torch.device('cpu'))
